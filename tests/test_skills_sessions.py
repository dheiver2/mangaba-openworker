...
def test_no_workspace_means_global_only(manager, tmp_path):
    ...
    (ws / ".mangaba" / "skills").mkdir(parents=True)
    _skill(ws / ".mangaba" / "skills", "local-only")
...
def test_empty_catalog_is_safe(tmp_path):
    from mangaba.tools.registry import ToolRegistry
...
def test_live_load_skill_semantics(manager):
    ...
    from mangaba.agents.registry import get_agent

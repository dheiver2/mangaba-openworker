...
from mangaba.sessions import SessionRecord
...
def test_disable_persona_archives_its_sessions(tmp_path):
    """Disable = "put this mangaba and its history away": the persona's real sessions are
...
def test_ws_rejects_oversized_message(tmp_path):
    ...
    from mangaba.attachments import MAX_ATTACHMENTS
...
def test_server_sets_explicit_websocket_frame_limit(tmp_path, monkeypatch):
    ...
    from mangaba.server import run as server_run
...
def test_standalone_server_token_file_is_user_only(tmp_path, monkeypatch):
    ...
    try:
        assert path == tmp_path / "mangaba-state" / "sidecar-9876.token"
        assert path.read_text().strip() == os.environ["MANGABA_API_TOKEN"]
        assert len(path.read_text().strip()) == 64
        assert (path.stat().st_mode & 0o777) == 0o600
    finally:
        path.unlink(missing_ok=True)
        os.environ.pop("MANGABA_API_TOKEN", None)
...
def test_sidecar_token_gates_rest_and_websockets(tmp_path, monkeypatch):
    from mangaba.mcp.config import global_mcp_path
    from starlette.websockets import WebSocketDisconnect as WSD

    monkeypatch.setenv("MANGABA_API_TOKEN", "a" * 64)
    manager = SessionManager(workspace=tmp_path, provider=ScriptedProvider([]))
    client = TestClient(create_app(manager))
    ...
    assert client.get(
        "/v1/sessions", headers={"X-Mangaba-Token": "wrong"}
    ).status_code == 401

    headers = {"X-Mangaba-Token": "a" * 64}
    ...
    with client.websocket_connect(
        "/ws/session/authed", subprotocols=["mangaba", "a" * 64]
    ) as ws:
        assert ws.accepted_subprotocol == "mangaba"
        assert ws.receive_json()["type"] == "ready"

    with client.websocket_connect(
        "/ws/events", subprotocols=["mangaba", "a" * 64]
    ) as ws:
        assert ws.accepted_subprotocol == "mangaba"

    # The MCP OAuth redirect callback (local flow) remains tokenless, then enforces
    # its own signed state. (The Mangaba Cloud /auth/callback and the broker
    # /oauth/callback were removed with the managed one-click flow.)
...
def test_workspace_command_trust_controls_live_engine(tmp_path):
    ...
    (proj / ".mangaba").mkdir(parents=True)
    (proj / ".mangaba" / "config.toml").write_text(
...
def test_google_descriptors_keep_manual_fields(tmp_path):
    """The managed one-click flow was removed (no Mangaba Cloud in this fork); every
    Google connector must still carry its manual credential fields so the manual paste
    path stays alive."""
    client = _client(tmp_path, [])

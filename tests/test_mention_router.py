...
per-thread mangaba session spawns (visible in the sidebar, thread-scoped standing
send_message grant); follow-up tags in the same thread steer the same session. A
channel with a user-connected (subscribed) mangaba overrides the router — it must
...
from mangaba.connectors.adapters import slack_event_to_event
from mangaba.connectors.base import MessageEvent, SessionSource
from mangaba.conversations import ConversationStore
...
def test_subscribed_mangaba_overrides_router(tmp_path, monkeypatch):
    mgr = _mgr(tmp_path)
    captured = _capture_deliveries(mgr, monkeypatch)
    ...
    # Delivered to the connected mangaba with must-respond framing + the thread target…

...
State is fully isolated: `MANGABA_STATE_DIR` redirects the SecretStore and the manager's data
...
from mangaba.interactions import decode
...
from mangaba.server import create_app
...
async def test_ui_refresh_cross_cutting_e2e(fake_slack, tmp_path, monkeypatch):
    ...
    # by what this test writes; the manager's own data dir lives under tmp_path/.mangaba.
    ...
    # machine-global ~/.config/mangaba (so this run cannot mutate the real secrets hash).

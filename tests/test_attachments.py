...
model actually reads the image (`MANGABA_LIVE_VISION=1`, key read from the SecretStore).
"""
...
from mangaba.attachments import build_user_content, content_to_text
...
async def test_image_reaches_provider_unmodified():
    from mangaba.agents.chat import chat_agent
...
def test_list_content_message_persists_and_titles(tmp_path):
    from mangaba.conversations import ConversationStore, title_from
...
@pytest.mark.skipif(
    os.environ.get("MANGABA_LIVE_VISION") != "1",
    reason="opt-in: real OpenAI vision call (set MANGABA_LIVE_VISION=1)",
)
def test_live_vision_model_reads_image():
    from mangaba.providers import OpenAIProvider
...
def test_pdf_attachment_invalid_or_oversized_skipped():
    from mangaba.attachments import MAX_PDF_CHARS

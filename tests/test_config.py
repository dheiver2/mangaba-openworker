...
def test_global_and_workspace_override(tmp_path):
    ...
    ws = tmp_path / "ws"
    (ws / ".mangaba").mkdir(parents=True)
    (ws / ".mangaba" / "config.toml").write_text(
        'max_iterations = 30\nmode = "plan"\n'
    )
...
def test_workspace_trust_is_canonical_and_user_owned(tmp_path):
    from mangaba.workspace_trust import WorkspaceTrustStore
...
def test_build_engine_honors_explicit_empty_command_allowlist(tmp_path):
    from mangaba.agent import build_code_engine
    from mangaba.config import global_config_path
...
def test_build_engine_respects_max_iterations(tmp_path):
    ...
    (tmp_path / ".mangaba" / "config.toml").write_text("max_iterations = 3\n")

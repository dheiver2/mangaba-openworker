...
from mangaba.connectors.tool_defs import TOOL_DEFS, approval_for_tool
from mangaba.connectors.tools import make_send_file_tool, make_send_message_tool
...
def test_browser_automation_reads_are_free_interactions_gate():
    from mangaba.connectors.browser_automation import make_browser_automation_tools
...
def _fake_roster(monkeypatch, channels_by_team: dict):
    from mangaba.connectors import slack_directory
...
def test_bare_channel_names_coerce_to_slack(tmp_path, monkeypatch):
    """The owner's exact transcript: the model sent target='all-mangaba' and
    '#all-mangaba' — no 'slack:' prefix — and got 'invalid target'. Bare names
    ...
    assert tool("all-mangaba", "Hi")["ok"] is True
    assert tool("#all-mangaba", "Hi")["ok"] is True
...
def test_unknown_ambiguous_and_not_member_names_error_actionably(tmp_path, monkeypatch):
    ...
    assert "invite @Mangaba" in tool("slack:#private-ops", "Hi")["error"]

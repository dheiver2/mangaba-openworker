...
from mangaba.connectors import (
    ConnectorSettings,
    FakeAdapter,
    Gateway,
    MessageEvent,
...
def test_send_message_success(tmp_path):
    ...
    assert tool.__mangaba_schema__["function"]["name"] == "send_message"
...
def test_load_settings_from_secretstore(tmp_path, monkeypatch):
    ...
    settings = __import__(
        "mangaba.connectors.config", fromlist=["load_settings"]
...
class _StubProvider:
    ...
    def complete(self, **_kw):  # pragma: no cover - never invoked at build time
        from mangaba.providers import AssistantTurn

        return AssistantTurn()

    def capabilities(self, _model):  # pragma: no cover
        from mangaba.providers.base import ModelCapabilities

        return ModelCapabilities()

    def stream(self, **_kw):  # pragma: no cover
        from mangaba.providers.base import StreamChunk
...
def test_connector_list_descriptors(tmp_path):
    from mangaba.connectors import connector_list
...
def test_connector_list_pre_connect_copy(tmp_path):
    ...
    from mangaba.connectors import connector_list
    from mangaba.connectors.catalog_copy import ACCESS
    from mangaba.connectors.descriptors import list_descriptors
...
def test_reconnect_does_not_clobber_secret_or_allowlist(tmp_path):
    ...
    from mangaba.connectors import connect_connector
    from mangaba.connectors.descriptors import get_descriptor
...
def test_connect_validation_runs(tmp_path):
    from mangaba.connectors import connect_connector
    from mangaba.connectors.descriptors import ValidationResult, get_descriptor
...
def test_telegram_message_mapper():
    ...
    from mangaba.connectors import telegram_message_to_event
...
def test_slack_event_mapper_and_loop_guard():
    from mangaba.connectors import slack_event_to_event
...
def test_make_adapter():
    from mangaba.connectors import SlackAdapter, TelegramAdapter, make_adapter
...
async def test_slack_resolves_and_caches_display_name():
    from mangaba.connectors import SlackAdapter
...
def test_new_connectors_connect_and_gate_tools(tmp_path):
    from mangaba.connectors import connect_connector, connector_list
    from mangaba.connectors.integration_tools import make_integration_tools
...
def _connected_tools(tmp_path, monkeypatch, calls):
    ...
    import mangaba.connectors.integration_tools as it
...
def test_registry_has_no_duplicate_names():
    ...
    from mangaba.connectors.tool_defs import TOOL_DEFS
...
def test_notion_read_page_flattens_blocks(tmp_path, monkeypatch):
    import mangaba.connectors.integration_tools as it
    from mangaba.connectors import accounts
...
def test_managed_callback_profile_keys_by_account_id(tmp_path):
    ...
    from mangaba.connectors import accounts
    from mangaba.connectors.setup import managed_connect_connector
...
def test_google_drive_multi_account_keys_by_email(tmp_path):
    ...
    assert ids == ["rohit@mangaba.app", "work@acme.com"], ids
...
def test_outlook_managed_multi_account_keys_by_email(tmp_path, monkeypatch):
    ...
    secrets = SecretStore(tmp_path / "secrets.json")
    for email, tok in (("rohit@mangaba.com", "g1"), ("ops@acme.com", "g2")):
    ...
    assert ids == ["ops@acme.com", "rohit@mangaba.com"], ids
    ...
    assert out["account"] == "rohit@mangaba.com"
...
def test_validate_whoami_helper(monkeypatch):
    import mangaba.connectors.descriptors as d
...
def test_experimental_hidden_until_enabled(tmp_path, experimental_descriptor):
    from mangaba.connectors import connector_list, set_experimental_enabled
...
def test_experimental_package_loads_cleanly():
    ...
    from mangaba.connectors.descriptors import DESCRIPTORS
    from mangaba.connectors.experimental import EXPERIMENTAL_DESCRIPTORS

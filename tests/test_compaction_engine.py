...
from mangaba.permissions import PermissionEngine
...
from mangaba.providers.base import TokenUsage
...
def test_summarizer_failure_unattended_auto_trims(tmp_path):
    ...
    assert compacted and "aparado" in compacted[0].data["text"].lower()
...
def test_summarizer_failure_attended_prompts_retry_then_succeeds(tmp_path):
    ...
    async def asker(args, tool_call_id=None):
        asked.append(args)
        return {"answer": "Tentar novamente"}

    engine.question_asker = asker
    collect(engine)

    assert asked and asked[0]["options"] == ["Tentar novamente", "Aparar os 10% mais antigos"]
...
def test_summarizer_failure_attended_choose_trim(tmp_path):
    ...
    async def asker(args, tool_call_id=None):
        return {"answer": "Aparar os 10% mais antigos"}
...
def test_compaction_state_survives_save_and_rebuild(tmp_path):
    from mangaba.compaction import CompactionState
    from mangaba.server.manager import SessionManager

...
from mangaba.skills import SkillLoader, SkillStore, validate_name
...
def test_create_project_scoped(store, workspace):
    ...
    md = workspace / ".mangaba" / "skills" / "release-checklist" / "SKILL.md"
    assert md.is_file()
...
def test_move_roundtrip(store, workspace):
    ...
    assert (workspace / ".mangaba" / "skills" / "mover" / "SKILL.md").is_file()
...
def test_move_collision_leaves_source(store, workspace):
    ...
    assert (workspace / ".mangaba" / "skills" / "both" / "SKILL.md").is_file()
...
from mangaba.skills import save_skill_tool  # noqa: E402
...
def test_save_skill_requires_approval_metadata(store):
    ...
    assert tool.__mangaba_schema__["function"]["name"] == "save_skill"
    required = tool.__mangaba_schema__["function"]["parameters"]["required"]

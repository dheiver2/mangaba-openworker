            ...
            while True:
                answer = await self._interruptible(
                    self.question_asker(
                        {
                            "question": (
                                "A compactação de contexto falhou — o resumidor não "
                                "conseguiu condensar o histórico desta sessão. Como devo prosseguir?"
                            ),
                            "options": ["Tentar novamente", "Aparar os 10% mais antigos"],
                            "allow_text": False,
                            "header": "Compactação",
                        },
                        None,
                ...
                if not answer or answer.get("answer") != "Tentar novamente":
                    break
                try:
                    state = await asyncio.to_thread(_build)
                    failed = False

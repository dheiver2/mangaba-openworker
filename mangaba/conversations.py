...
Layout under a base dir (default `~/.config/mangaba/`):
  mangaba.db                  SQLite index: sessions(id → project, title, n_msgs), workspaces, memory
...
class ConversationStore:
    def __init__(self, base_dir: str | Path) -> None:
        ...
        self.db_path = self.base / "mangaba.db"
    ...
    def save(self, record: SessionRecord) -> None:
        ...
        with self._lock:
            ...
            self._conn.execute(
                """
                INSERT INTO sessions (session_id, workspace, model, mode, title, agent, n_msgs, messages, extra_roots, grants, compaction, plan, updated_at)
    ...
    def load(self, session_id: str) -> Optional[SessionRecord]:
        ...
        return SessionRecord(
            ...
            plan=_load_roots(row["plan"] if "plan" in row.keys() else None),

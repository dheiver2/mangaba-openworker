...
Global:    ~/.config/mangaba/mcp.json
Workspace: <workspace>/.mangaba/mcp.json   (overrides global on name clash)
...
def _config_paths(workspace: Optional[str | Path]) -> list[Path]:
    ...
    if workspace:
        paths.append(Path(workspace).expanduser() / ".mangaba" / "mcp.json")
    return paths

...
def state_dir() -> Path:
    """Where mangaba keeps its state — the one cross-platform source of truth.

    Resolution order:
    1. `$MANGABA_STATE_DIR` — explicit override on any OS (used by tests/sidecars).
    2. Windows: `%APPDATA%\\mangaba` (e.g. `C:\\Users\\You\\AppData\\Roaming\\mangaba`),
       the native per-user app-data location.
    3. macOS / Linux: `~/.config/mangaba` (XDG-style, unchanged from prior behavior).
    """
    base = os.environ.get("MANGABA_STATE_DIR")
    if base:
        return Path(base).expanduser()
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        if appdata:
            return Path(appdata) / "mangaba"
    return Path.home() / ".config" / "mangaba"

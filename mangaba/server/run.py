"""Launch the server with uvicorn. Used by the desktop GUI sidecar and `mangaba-server`."""
...
def _exit_when_orphaned() -> None:
    """When launched as a desktop sidecar (`MANGABA_EXIT_WITH_PARENT=1`), exit if the parent
    process dies — even on an abrupt kill (e.g. the Tauri dev watcher restarting the app, or a
    crash) that skips the shell's graceful child-kill. Standalone `mangaba-server` runs are
    unaffected.

    The GUI passes its own PID in `MANGABA_PARENT_PID`. Watching that explicit PID (not
    ...
    *grandchild* of the GUI — the bootloader sits in between, so getppid() points at the
    ...
    """
    if os.environ.get("MANGABA_EXIT_WITH_PARENT") != "1":
        return
    import threading

    try:
        parent = int(os.environ.get("MANGABA_PARENT_PID") or 0)
    except ValueError:
        parent = 0
...
def _ensure_api_token(port: int) -> Path | None:
    ...
    if os.environ.get("MANGABA_API_TOKEN"):
        return None  # Tauri supplied an in-memory token; never persist it.
    token = secrets.token_hex(32)
    os.environ["MANGABA_API_TOKEN"] = token
...
def main(argv=None) -> None:
    ...
    parser = argparse.ArgumentParser(prog="mangaba-server")
    ...
    os.environ["MANGABA_PORT"] = str(args.port)

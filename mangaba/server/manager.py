...
logger = logging.getLogger("mangaba.manager")
...
class SessionManager:
    def __init__(
        ...
        *,
        ...
        if data_dir is not None:
            ...
            base = Path(self.default_workspace) / ".mangaba"
        else:
            base = state_dir()
        base.mkdir(parents=True, exist_ok=True)

        self.memory_store: MemoryStore = SQLiteMemoryStore(base / "mangaba.db")
        self.audit_store = AuditStore(base / "mangaba.db")
    ...
    def set_persona_enabled(self, persona_id: str, enabled: bool) -> dict[str, Any]:
        ...
        non-internal) sessions — disable means "put this mangaba and its history away", so
    ...
    def list_artifacts(self, session_id: str) -> list[dict[str, Any]]:
        ...
        # TCC prompt ("Mangaba would like to access data from other apps") on every turn.
    ...
    def pick_native_folder(self) -> dict[str, Any]:
        ...
        if sys.platform == "darwin":
            cmd = [
                ...
                "-e",
                'POSIX path of (choose folder with prompt "Give the mangaba access to a folder")',
            ]
        elif sys.platform == "win32":
            ...
            ps = (
                ...
                "$f.Description = 'Give the mangaba access to a folder'; "
    ...
    def get_settings(self) -> dict[str, Any]:
        ...
        return {
            ...
            # hardcoded POSIX one (Windows -> %APPDATA%\mangaba, macOS/Linux -> ~/.config).
    ...
    def context_bar(self) -> bool:
        """Se o composer mostra a barra de preenchimento da janela de contexto. DESLIGADA
        por padrão (pedido do dono): o chip então mostra o total da sessão, e o popover
        mantém os dois números."""
    ...
    def set_compaction_settings(
        ...
        if threshold_pct is not None:
            try:
                ...
                return {"ok": False, "error": "compaction_threshold_pct precisa ser um número"}
            if not 0.10 <= pct <= 0.95:
                return {
                    "ok": False,
                    "error": "compaction_threshold_pct precisa estar entre 0.10 e 0.95",
                }
    ...
    def set_scratch_base(self, path: str) -> dict[str, Any]:
        ...
        created (default ~/Mangaba). The raw value is stored so the UI shows it as entered;
    ...
    async def refresh_gateway(self) -> list[str]:
        ...
        print(f"[mangaba] messaging gateway reloaded: {', '.join(started) or 'none'}")
        return started
    ...
    async def _dispatch_inbound(self, event) -> None:
        ...
        if src.chat_type in ("channel", "group"):
            ...
            # the per-thread mangaba session.
    ...
    async def _route_mention(self, event, ms: MessageSource, subs) -> None:
        """@Mangaba tagged in a channel. A subscribed (user-connected) mangaba owns the channel
        and must answer; otherwise the per-thread mangaba session handles it — spawned on the
    ...
    async def _spawn_mention_session(
        ...
        """First tag in a thread: a NEW visible mangaba session that owns the thread. Its

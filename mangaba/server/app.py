...
def _browser_page(
    ...
    return (
        ...
        f"<title>{_html.escape(title)} — Mangaba</title><style>"
        ...
        "</style></head><body>"
        '<div class="card"><div class="mark"><i></i>Mangaba</div>'
        f"{icon}<h1>{_html.escape(title)}</h1><p>{_html.escape(detail)}</p>{err}</div>"
        '<div class="foot">Served locally by Mangaba on your Mac</div>'
        "</body></html>"
    )
...
_CONNECT_FAILED_DETAIL = (
    ...
    "Close this tab and try again from Mangaba."
)
...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    async def lifespan(_app: FastAPI):
        try:
            ...
            if live:
                print(f"[mangaba] messaging gateway live: {', '.join(live)}")
    ...
    def cloud_gallery_detail(slug: str) -> dict[str, Any]:
        """Solo page for one gallery mangaba: publisher pitch + capabilities
    ...
    async def mcp_oauth_callback(
        ...
        if error:
            return HTMLResponse(
                _browser_page(
                    "Sign-in failed",
                    "The service reported an error. Return to Mangaba and try again.",
                    ok=False,
                    error=error,
                ),
                status_code=400,
        ...
        if not code or not mcp_oauth.deliver_callback(code, state or None):
            return HTMLResponse(
                _browser_page(
                    ...
                    "The sign-in may have timed out. Return to Mangaba and start it again.",
                    ok=False,
                ),
                status_code=400,
            )
        return HTMLResponse(
            _browser_page(
                "Connected",
                "Sign-in complete. You can close this tab and return to Mangaba.",
                ok=True,
            )
        )
    ...
    # -- Mangaba Cloud: sign-in + managed one-click connect ---------------
    ...
    async def cloud_auth_callback(code: str = "", state: str = "", error: str = ""):
        ...
        signin_failed_detail = (
            "Close this tab and try signing in again from Mangaba."
        )
        if error:
            return HTMLResponse(
                _browser_page(
        ...
        return HTMLResponse(
            _browser_page(
                "Signed in",
                "You're signed in to Mangaba Cloud. "
                "You can close this tab and return to Mangaba.",
            )
        )
    ...
    def settings_set_context_bar(body: dict) -> dict[str, Any]:
        # Composer: mostra a barra de preenchimento da janela de contexto, ou só o popover.
    ...
    def settings_set_compaction(body: dict) -> dict[str, Any]:
        # Overrides da auto-compactação (OPE-27): % da janela de contexto, teto absoluto
        # de tokens e o modelo resumidor ("" → o modelo da própria sessão).
        b = body or {}
    ...
    if os.environ.get("MANGABA_DEBUG_INJECT") == "1":
    ...
    async def ws_session(ws: WebSocket, session_id: str) -> None:
        ...
        if engine is None:
            ...
            await ws.close()
            return
        # Prompt de falha da auto-compactação (OPE-27): só uma sessão ATENDIDA pode ser
                ...
                if kind == "approval":
                    ...
                    if skill is not None:
                        if not isinstance(skill, str) or not skill.strip():
                            await reject_input("Skill inválida: era esperado um nome.")
                            continue
                        skill = skill.strip()
                        menu = manager.effective_skill_names(session_id, workspace)
                        if skill not in menu:
                            await reject_input(
                                f"A skill '{skill}' não está disponível nesta sessão."
                            )
                            continue

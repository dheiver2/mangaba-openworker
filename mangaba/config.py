...
Workspace: <workspace>/.mangaba/config.toml   (overrides global)
...
class Config:
    ...
    # Mangaba Cloud (sign-in + managed connectors). Config, never constants:
    # dev/staging/BYO-VPC deployments point these at their own instances.
    cloud_base_url: str = "https://api.mangaba.com"
    ...
    cloud_auth_domain: str = "mangaba.us.auth0.com"
    cloud_client_id: str = "g1l4Q1lhYWmyS03qPSf4KEJGrgq02Qam"
    cloud_audience: str = "https://api.mangaba.app"
...
def workspace_allowed_commands(workspace: str | Path) -> list[str]:
    ...
    path = Path(workspace).expanduser() / ".mangaba" / "config.toml"
...
def load_config(
    ...
    *,
    ...
    if workspace:
        w = Path(workspace).expanduser() / ".mangaba" / "config.toml"
        if w.is_file():

...
function sidecarToken(): string {
  const state =
    process.env.MANGABA_STATE_DIR ||
    (process.platform === "win32"
      ? join(process.env.APPDATA || homedir(), "mangaba")
      : join(homedir(), ".config", "mangaba"));
  try {
...
export async function scratchBaseIfReady(): Promise<string | null> {
  try {
    ...
    if (res.ok && s.model_ready) {
      return String(s.scratch_base || "~/Mangaba").replace(/^~(?=\/|$)/, homedir());
    }
  } catch {
    /* backend unreachable */
  }
...
export async function sendTask(page: Page, text: string) {
  await page.getByPlaceholder(/Ask the mangaba/).fill(text);

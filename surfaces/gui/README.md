# mangaba GUI (React + Tauri)

A thin client of the mangaba server (OpenAI-compatible API + WS event/approval stream).
Same codebase runs in a browser (dev) and as the Mangaba desktop app.
...
1. **Start the server** (needs a model key, e.g. `OPENAI_API_KEY`, in the environment —
   ...
   ```bash
   cd platform
   ./.venv/bin/mangaba-server --cwd /path/to/your/project --port 8765
   ```
2. **Start the UI:**
   ```bash
...
`VITE_MANGABA_HTTP` / `VITE_MANGABA_WS`). Start the server before Vite so the
...
in dev it finds the server at `platform/.venv/bin/mangaba-server` automatically (a

...
test("skills-session: new skill offered in '/', disabled one absent", async ({ page }) => {
  ...
  await page.getByLabel("Nome").fill("fresh-skill");
  await page.getByLabel("Instruções").fill("Do the fresh thing.");
  ...
  await expect(page.getByRole("status")).toContainText("desativada em todo lugar");

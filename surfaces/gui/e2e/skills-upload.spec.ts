...
test("skills-upload: preview installs nothing → confirm → uploaded badge", async ({ page }) => {
  ...
  await page.getByText("Importar um arquivo").click();
  await page.getByLabel("Enviar um arquivo de skill").setInputFiles({
    name: "greet.zip",
  ...
  await expect(page.getByText("Revise antes de instalar")).toBeVisible();
  ...
  await page.getByRole("button", { name: "Instalar skill" }).click();

...
async function openOnboarding(page) {
  ...
  await page.getByRole("button", { name: "Rodar a configuração de novo" }).click();
...
test("key form: Test verifies, saves, and returns to the gallery with the ✓", async ({
  ...
  await expect(page.getByRole("heading", { name: "Bem-vindo ao Mangaba" })).toBeVisible();
  ...
  await expect(page.getByTestId("ob-provider-zai")).toContainText("✓ Conectado", {
    timeout: 5_000,
  });
  await expect(page.getByTestId("ob-continue")).toBeEnabled();
});
...
test("tools page: sign-in morphs the page into the connector gallery; a card connects one-click", async ({
  ...
  await expect(page.getByText("Um chat só aconselha")).toBeVisible();
  await expect(page.getByTestId("ob-tool-outlook")).toContainText("Fique em dia com o e-mail");
  await expect(page.getByTestId("ob-tool-outlook").getByRole("button")).toHaveCount(0);
  await expect(page.getByTestId("ob-tool-attio")).toContainText("Acompanhe cada relacionamento");
  await expect(page.getByTestId("ob-tool-google-soon")).toContainText("Em breve");
  await expect(page.getByText("Entre para ter conexões com um clique")).toBeVisible();
  await expect(page.getByTestId("ob-tools-skip")).toContainText("Continuar sem entrar");
  ...
  await expect(page.getByTestId("ob-tools-signedin")).toContainText("Você está conectado");
  await expect(
    page.getByTestId("ob-tool-attio").getByRole("button", { name: "Conectar" }),
  ).toBeVisible();
  ...
  await page.getByTestId("ob-tool-outlook").getByRole("button", { name: "Conectar" }).click();
  await expect(page.getByTestId("ob-tool-outlook")).toContainText("✓ Conectado", {
    timeout: 10_000,
  });

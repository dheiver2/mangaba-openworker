...
test("scheduled run session shows the run banner; Back returns to the task detail", async ({
  ...
  await expect(page.getByText("Cada execução é uma conversa ao vivo", { exact: false })).toBeVisible();

  // Open the running run: a normal session view, but with the automation-context banner.
  await page.getByTitle("Abrir a conversa desta execução").click();
  const banner = page.getByTestId("run-banner");
  await expect(banner).toBeVisible();
  await expect(banner).toContainText("Execução agendada");
  await expect(banner).toContainText("Daily AI News");

  // Back link lands on the SAME task's detail, not the bare list.
  await banner.getByRole("button", { name: "← Voltar às execuções" }).click();
  await expect(page.getByRole("button", { name: /Executar agora/ })).toBeVisible();

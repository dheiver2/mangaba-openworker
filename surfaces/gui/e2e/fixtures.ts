...
const SETTINGS = {
  ...
  nav_layout: "grouped",
  scratch_base: "~/Mangaba",
  secrets_path: "/Users/test/.config/mangaba/secrets.json",
  sessions_peek: 5,
  context_bar: false,
...
const PERSONAS = {
  personas: [
    { id: "cowork", name: "Mangaba", icon: "cowork", tagline: "Produce a deliverable — research, analysis, scripts", needs_workspace: true, builtin: true, family: "knowledge", workspace: "deliverable", tools: ["files", "search"], enabled: true, surfaced: true, default: true },
    ...
    { id: "ops", name: "Ops Mangaba", icon: "wrench", tagline: "Operate and investigate — runbooks, logs, infrastructure", needs_workspace: true, builtin: true, family: "knowledge", workspace: "deliverable", tools: ["files", "shell"], enabled: true, surfaced: true, default: false },
    ...
    { id: "acme-notes", name: "Acme Notes", icon: "pencil", tagline: "Acme's note-taking mangaba", needs_workspace: true, builtin: false, family: "knowledge", workspace: "deliverable", tools: ["files"], enabled: false, surfaced: false, default: false },
  ],
};
...
const PINNED_SESSION = {
  ...
  workspace: "/Users/test/Mangaba/launch-note",
  agent: "cowork",
  model: "anthropic:claude-opus-4-8",
  mode: "interactive",
...
// Seven unpinned Mangaba sessions: enough to exercise the sidebar peek cap (5) + "Show more (2)".
...
const OPS_SESSION = {
  session_id: "ops-1",
  title: "Ops triage",
  workspace: "/Users/test/Mangaba/ops-triage",
  agent: "ops",
  model: "anthropic:claude-opus-4-8",
  mode: "interactive",
...
const GALLERY_PERSONAS = [
  {
    ...
    description: "A sales-focused mangaba.",
    family: "knowledge",
  ...
  },
  {
    ...
    description: "A recruiting mangaba.",
    family: "knowledge",
    workspace: "deliverable",
    publisher: "Mangaba",
...
const PRIMARY_ROOT = { path: "/Users/test/Mangaba/launch-note", writable: true, label: "scratch", primary: true, exists: true };
...
const PROVIDERS = [
  ...
  // mangaba: keyless local provider — "configured" without proving anything runs; the
  // onboarding gallery shows "No key needed" and its form is endpoint + Detect (§39).
  { name: "mangaba", title: "Mangaba (local models)", needs_key: false, fields: [{ key: "base_url", label: "Endpoint", secret: false, required: false, help: "", placeholder: "http://127.0.0.1:11434", default: "http://127.0.0.1:11434" }], configured: true, values: {}, suggested_models: ["qwen3-coder:30b"], key_set_at: null, last_used_at: null },
];
...
export async function mockApi(page: import("@playwright/test").Page) {
      ...
      if (msg.type === "user_message") {
        ...
        if (/run a tool/i.test(msg.text)) {
          ...
          send("permission_required", {
            ...
            reason: "The mangaba wants to run a command.",
          });
        ...
        if (/compact the context/i.test(msg.text)) {
          send("compacting", {});
          setTimeout(() => {
            send("compacted", { text: "Contexto compactado — os turnos mais antigos foram resumidos" });
            send("assistant_message", { text: "Continuando de onde parei." });
            send("turn_done");
          }, 400);
          return;
        }
  ...
  await page.route("**/v1/**", async (route) => {
    ...
    if (/\/v1\/providers\/[^/]+$/.test(p) && m === "DELETE") {
      ...
      prov.configured = !prov.needs_key; // keyless (mangaba) stays "configured"

...
test("Send-to-Inbox toggle (in the Mode menu) flips and persists across a reload", async ({
  ...
  const sw = page.getByRole("switch", { name: "Enviar aprovações para a caixa de entrada" });
  ...
  await page.reload();
  await openModeMenu(page);
  await expect(page.getByRole("switch", { name: "Enviar aprovações para a caixa de entrada" })).toHaveAttribute(
    "aria-checked",
    "true",
  );
});
...
test("unattended: a tool request parks (no inline approval card)", async ({ page }) => {
  await page.goto("/");
  await openModeMenu(page);
  await page.getByRole("switch", { name: "Enviar aprovações para a caixa de entrada" }).click();
  ...
  await expect(page.getByText("The mangaba wants to run a command.")).toHaveCount(0);
});

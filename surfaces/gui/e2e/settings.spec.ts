...
test("Settings opens as a full page and navigates sections", async ({ page }) => {
  ...
  for (const label of ["Geral", "Modelos", "Entrada de voz"]) {
    await expect(page.getByRole("button", { name: label, exact: true })).toBeVisible();
  }
  // Folded/hidden tabs: Files is a General card now; Personas is launch-flagged off.
  await expect(page.getByRole("button", { name: "Arquivos", exact: true })).toHaveCount(0);
...
test("Settings: Personas tab returns behind the launch flag", async ({ page }) => {
  ...
  await expect(page.getByText("Adicionar personas")).toBeVisible();
});
...
test("Models: provider gallery states; vendor form previews models", async ({ page }) => {
  ...
  await expect(picker).toContainText("No seletor do compositor");
  ...
  await expect(preview).toContainText("Modelos inclusos");
...
// affordance; removing reverts the card to "Não configurado".
test("Models: Remove key reverts a configured provider", async ({ page }) => {
  ...
  await expect(page.getByTestId("set-provider-anthropic")).toContainText("Não configurado");
});
...
test("Settings: Token savings card edits PDF fallback and thresholds", async ({ page }) => {
  ...
  await expect(card.getByText("Economia de tokens")).toBeVisible();

  // Fallback mode: fixture says "text"; switching marks "Send page images" active.
  const seg = page.getByTestId("pdf-fallback");
  await expect(seg.getByRole("button", { name: "Extrair texto" })).toHaveClass(/active/);
  const [req] = await Promise.all([
    page.waitForRequest((r) => r.url().endsWith("/v1/settings/pdf") && r.method() === "POST"),
    seg.getByRole("button", { name: "Enviar imagens das páginas" }).click(),
  ]);
  expect(req.postDataJSON()).toEqual({ pdf_fallback: "images" });
  await expect(seg.getByRole("button", { name: "Enviar imagens das páginas" })).toHaveClass(/active/);

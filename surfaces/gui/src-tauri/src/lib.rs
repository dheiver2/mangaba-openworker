//! Mangaba desktop shell.
//!
//! Tauri is a thin native window over the existing React SPA. It:
//!   1. picks a free localhost port and starts the Python `mangaba-server` as a managed
...
//! comes from the SecretStore (Settings tab), see `mangaba.providers.resolve_api_key`.
...
use mangaba_stt::{Dictation, DownloadProgress};
use serde::Serialize;
use tauri::{
    menu::{Menu, MenuItem},
...
///   1. `MANGABA_SERVER_BIN` env override.
...
///   3. Legacy onefile slot: `mangaba-server[.exe]` next to the app binary (pre-onedir
///      builds used Tauri externalBin).
///   4. Dev fallback: the repo venv, relative to this crate (`src-tauri` → `platform/.venv`;
///      `bin/` on POSIX, `Scripts\` on Windows).
fn server_bin() -> PathBuf {
    if let Ok(p) = std::env::var("MANGABA_SERVER_BIN") {
    ...
    if cfg!(windows) {
        p.push("../../../.venv/Scripts/mangaba-server.exe");
    } else {
        p.push("../../../.venv/bin/mangaba-server");
    }
    p
}

/// Mirror of `mangaba.secrets.state_dir()` so the shell and server agree on `desktop.json`.
/// Windows: `%APPDATA%\mangaba`; POSIX: `~/.config/mangaba`. `MANGABA_STATE_DIR` overrides.
fn state_dir() -> PathBuf {
    if let Ok(d) = std::env::var("MANGABA_STATE_DIR") {
        return PathBuf::from(d);
    }
    #[cfg(windows)]
    {
        if let Ok(appdata) = std::env::var("APPDATA") {
            return PathBuf::from(appdata).join("mangaba");
        }
    }
    let home = std::env::var("HOME").unwrap_or_else(|_| ".".into());
    PathBuf::from(home).join(".config").join("mangaba")
}
...
/// The sidecar's log file: `<state_dir>/logs/mangaba-server.log`, fresh per
...
fn server_log_file() -> Option<std::fs::File> {
    ...
    let path = dir.join("mangaba-server.log");
    if path.exists() {
        let _ = std::fs::rename(&path, dir.join("mangaba-server.log.old"));
    }
...
async fn install_update(
    ...
    // Exit path's sidecar kill runs via RunEvent, so no orphaned mangaba-server.
    app.restart();
}
...
pub fn run() {
    ...
    let inject = format!(
        "window.__MANGABA_HTTP__={http:?};window.__MANGABA_WS__={ws:?};window.__MANGABA_API_TOKEN__={api_token:?};window.__OCW_PLATFORM__={:?};",
        std::env::consts::OS
    );
    ...
    tauri::Builder::default()
        ...
        // duplicate sidecar — which previously left a window stuck on "Starting mangaba…".
        ...
        .setup(move |app| {
            ...
            server_cmd
                ...
                .env("MANGABA_EXIT_WITH_PARENT", "1")
                .env("MANGABA_PARENT_PID", std::process::id().to_string())
                .env("MANGABA_API_TOKEN", &api_token)
                ...
                // (the "Starting mangaba…" freeze on Windows). Hand it real handles: the
            ...
            let child = match server_cmd.spawn() {
                Ok(child) => Some(child),
                Err(e) => {
                    eprintln!("[mangaba] failed to start server sidecar: {e}");
                    None
                }
            };
            ...
            let open_i = MenuItem::with_id(app, "open", "Open Mangaba", true, None::<&str>)?;
            ...
            TrayIconBuilder::new()
                ...
                .on_menu_event(|app, event| match event.id.as_ref() {
                    ...
                    "settings" => {
                        ...
                        if let Some(w) = app.get_webview_window("main") {
                            let _ = w.eval(
                                "window.dispatchEvent(new CustomEvent('mangaba:open-settings'))",
                            );
                        }
                    }
                    "quit" => app.exit(0),

...
export function useRoots(sessionId: string, reloadKey?: number) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mangaba:roots-changed", onChanged);
    return () => window.removeEventListener("mangaba:roots-changed", onChanged);
  }, [sessionId, reload]);
  ...
  const apply = (res: { ok: boolean; error?: string; roots?: RootInfo[] }): boolean => {
    if (res.ok && res.roots) {
      setRoots(res.roots);
      setError("");
      window.dispatchEvent(new CustomEvent("mangaba:roots-changed", { detail: sessionId }));
      return true;
    }

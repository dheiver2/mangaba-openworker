...
function relayHealth(slack: SlackStatus | null): { dot: string; text: string } {
  if (!slack) return { dot: "bg-ok", text: "Ativo · relay gerenciado" };
  if (!slack.signed_in)
...
export function SlackDetail({ c, slack, onChanged }: DetailProps) {
  ...
  return (
    <div data-testid="slack-workspaces">
              ...
              <>
                ...
                <span data-testid="slack-mode-badge">
                  {relay
                    ...
                    : "Conectado · Socket Mode (tokens manuais)"}
                </span>
              </>
            ) : (
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            One @ocw app, installed per workspace — each keeps its own allow-list. Conecte
            colando os dois tokens do Socket Mode (bot xoxb-… e app xapp-…).
          </div>
        </div>
      )}
      ...
      {c.connected && (
        <div className={FOOT + " mt-2"}>Os nomes vêm do Slack automaticamente. Os IDs aparecem ao passar o mouse.</div>
      )}

      {adding && (
        <AddConnectionModal
          c={c}
          title="Adicionar um workspace"
          ...
          <>
            <div className={ROW}>
              <span className="min-w-0 flex-1 text-[12.5px] text-muted flex items-center gap-2 flex-wrap">
                <span>Ninguém autorizado ainda — as menções ao bot aparecem aqui para o seu OK.</span>
...
function DisconnectBtn({ teamId, busy, onClick }: { teamId: string; busy: boolean; onClick: () => void }) {
  return (
    <button
      ...
      title="Interrompe o relay deste workspace para este computador. O app continua instalado no Slack."
      onClick={onClick}
      disabled={busy}
    >
      {busy ? "Desconectando…" : "Desconectar workspace"}
    </button>
  );
}
          ...
          >
            ...
            {protectedIds?.includes(u) ? (
              <span
                ...
                title="Remova o acesso de responsável por aprovações antes de remover esta pessoa."
              >
                · owner
              </span>
            ) : (
              <button className={XBTN} title="remover" onClick={() => onRemove(u)}>
                ×
              </button>
            )}
          </span>
  ...
  return (
    <span className="relative" ref={wrap}>
      <button
        ...
        title="Escolher no diretório do workspace"
        onClick={toggle}
      >
        {buttonLabel}
      </button>
      {open && (
        ...
        >
          ...
          <div className="px-2 pb-1 text-[10.5px] text-faint">
            Do diretório do seu workspace — fica neste computador.
          </div>
        </div>
      )}
    </span>
...
function ApprovalOwnersRow({
  ...
  return (
    <div className={ROW} data-testid="slack-approval-owners">
      <span className={LABEL}>Aprovações</span>
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        {owners.length === 0 && (
        ...
        {owners.map((u) => (
          ...
          >
            ...
            {editable && (
              <button className={XBTN} title="remover responsável por aprovações" onClick={() => remove(u)}>
                ×
              </button>
            )}
          </span>
        ...
        {!editable && owners.length > 0 && (
          <span className="text-[11.5px] text-faint">Definido por quem instalou no workspace.</span>
        )}
  ...
  return (
    <div className={ROW + " bg-warnSoft/25"} data-testid={`waiting-${m.id}`}>
      ...
      <button
        ...
        title="Permitir o remetente e entregar esta mensagem agora"
      ...
      <button
        ...
        title="Permitir o remetente; esta mensagem é descartada"
        onClick={() => act("allow")}
      >
        Allow
      </button>

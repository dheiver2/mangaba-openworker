...
export function ConnectorsList({
  ...
  return (
    <div>
      ...
      {connected.length > 0 && (
        <>
          <div className={GRP_H + " !mt-0"}>Conectados · {connected.length}</div>
          <div className={GRP}>
            {connected.map((c) => (
              <button
                key={c.name}
      ...
      <div className={GRP_H}>Disponíveis</div>
      <div className={GRP}>
        {shown.map((c) => (
        ...
        {shown.length === 0 && (
          <div className={ROW + " text-[12.5px] text-muted"}>Nenhum resultado.</div>
        )}
      </div>
      {!showAll && !q && available.length > AVAILABLE_FOLD && (
        <div className={FOOT}>
          mais {available.length - AVAILABLE_FOLD} ·{" "}
...
function statusLine(c: Connector): string {
  ...
  if ((c.accounts?.length ?? 0) > 1) return `${c.accounts!.length} contas`;
  if ((c.portals?.length ?? 0) > 1) return `${c.portals!.length} portais`;
  if (c.auth === "none") return "Nativo";
  return c.account || "Conectado";
}
...
function healthChip(c: Connector, slack: SlackStatus | null) {
  ...
  if (c.name === "slack" && c.mode === "relay" && slack) {
    if (!slack.signed_in) return <span className={CHIP_WARN}>● Login necessário</span>;
    if (slack.relay.state === "offline") return <span className={CHIP_OFF}>● Offline</span>;
    if (slack.relay.state === "reconnecting")
      return <span className={CHIP_WARN}>● Reconectando</span>;
    if (Object.values(slack.teams).some((t) => !t.token_ok))
      return <span className={CHIP_WARN}>⚠ Token</span>;
    return <span className={CHIP_OK}>● Ativo</span>;
  }
  if (c.two_way && c.connected) return <span className={CHIP_OK}>● Ativo</span>;
  return <span className={CHIP_OK}>● Pronto</span>;
}

...
export function SkillsTab({
  ...
  const CONFIRMATION = "— o Mangaba já pode usá-la em todas as conversas.";
  const OFF_NOTE =
    "desativada em todo lugar. Se uma conversa já a usou, inicie uma nova para começar do zero.";
  const DELETE_NOTE =
  ...
  return (
    <section>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-[12.5px] text-muted mt-1 leading-relaxed">
            Instruções reutilizáveis que o Mangaba pode seguir em todas as conversas. Desligada
            aqui significa desligada em todo lugar.
          </p>
        </div>
        ...
        <div className="relative shrink-0">
          ...
          >
            <span className="inline-flex items-center gap-1.5">
              <Icon name="plus" size={13} /> Adicionar skill
            </span>
          </button>
          {addOpen ? (
            <>
              ...
              >
                ...
                >
                  <div className="text-[13px] font-medium">Escrever eu mesmo</div>
                  <div className="text-[11.5px] text-muted">
                    Um nome, uma descrição e as instruções
                  </div>
                </button>
                <button
                  role="menuitem"
                ...
                >
                  <div className="text-[13px] font-medium">Importar um arquivo</div>
                  <div className="text-[11.5px] text-muted">
                    Um .zip ou SKILL.md que alguém compartilhou — você revisa antes de instalar
                  </div>
                </button>
                <button
                  role="menuitem"
                ...
                >
                  <div className="text-[13px] font-medium">Criar com o Mangaba</div>
                  <div className="text-[11.5px] text-muted">
                    Inicia uma conversa — o Mangaba a constrói e pergunta antes de adicioná-la às
                    suas skills
                  </div>
                </button>
              </div>
      ...
      <input
        ...
        accept=".zip,.md"
        className="hidden"
        aria-label="Enviar um arquivo de skill"
        onChange={(e) => {
      ...
      {upload ? (
        <div className={`${CARD} p-4 mb-4`}>
          <div className="text-[13px] font-medium mb-1">Revise antes de instalar</div>
          <p className="text-[12.5px] text-muted mb-3">
            Leia as instruções — instalar uma skill significa que o Mangaba vai segui-las.
          </p>
          <div className="text-[13px] mb-1">
            <span className="font-medium">{upload.name}</span>
            <span className="text-muted"> — {upload.description || "sem descrição"}</span>
          </div>
          ...
          {upload.files?.length ? (
            <div className="text-[12px] text-muted mb-2">
              Arquivos incluídos: {upload.files.join(", ")}
            </div>
          ) : null}
      ...
      {editor ? (
        <div className={`${CARD} p-4 mb-4`}>
          <div className="text-[13px] font-medium mb-3">
            {editor.mode === "new" ? "Nova skill" : `Editar ${editor.name}`}
          </div>
          ...
          <input
            ...
            placeholder="Uma linha que o Mangaba usa para decidir quando ela se aplica"
          ...
          <textarea
            ...
            placeholder={"1. Reúna as novidades da última semana\n2. Escreva o relatório, com menos de 300 palavras"}
      ...
      <div className={`${CARD} divide-y divide-line`}>
        ...
        {rows.map((row) => (
          <div key={row.name} className="flex items-center gap-3 px-4 py-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {row.files ? (
                  ...
                  >
                    <Icon name="folder" size={11} /> {row.files} arquivo{row.files === 1 ? "" : "s"}
                  </button>
                ) : null}
              </div>
            ...
            >
              {armedDelete === row.name ? "Confirmar exclusão" : <Icon name="trash" size={13} />}
            </button>
            <label className="inline-flex items-center gap-1.5 text-[12px] text-muted">
              <input
                type="checkbox"
                role="switch"
                aria-label={`${row.name} ativada`}
                checked={row.enabled}
                onChange={(e) => {

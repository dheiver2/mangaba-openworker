...
// Coladas longas engolem a transcrição (pedido do dono, 2026-07-30): acima de um limite
// generoso, o balão recolhe com um mais…/menos…. Mensagem digitada normal nunca vê o
// controle; o texto completo continua valendo para copiar (BubbleMeta) e é o que o
// modelo recebeu.
...
function approvalChip(resolved: ApprovalDecision | undefined) {
  if (resolved === "deny")
    return <span className="text-[10.5px] px-1.5 rounded-full bg-dangerSoft text-danger shrink-0">✕ recusado</span>;
  return (
    <span
      className="text-[10.5px] px-1.5 rounded-full bg-okSoft text-ok shrink-0"
      title={resolved ? `aprovado · ${resolved.replace(/_/g, " ")}` : "aprovado"}
    >
      ✓ aprovado
    </span>
  );
      ...
      <div className="group flex items-baseline gap-2 px-2 py-0.5 rounded-lg hover:bg-paper" data-testid="turn-step">
        ...
        <LineText
          line={
            ...
            tool.name === "load_skill" && tool.preview?.includes('"error"')
              ? { pre: "Tentou a skill: ", obj: String(tool.args?.name ?? ""), post: " — indisponível" }
        ...
        {!!tool.standingRule && (
          <span
            ...
            title={`Permitido automaticamente pela aprovação permanente desta automação: ${tool.standingRule}. Revogue na página de Automações.`}
          >
        ...
        {!!tool.hidden && (
          <span
            ...
            title="Removido pelos seus filtros de privacidade antes de o agente ver os resultados — os agentes não recebem nenhum vestígio disso."
          >
            {tool.hidden} ocultos
          </span>
        )}
...
function TurnGroup({
  ...
  const stepsLabel = `${nSteps} passo${nSteps === 1 ? "" : "s"}`;
  ...
  return (
    <details className="stepgroup" open={open}>
      ...
      >
        ...
        <span>
          <span>{running ? `Executando ${stepsLabel}…` : stepsLabel}</span>
          {declined > 0 && (
            <>
              {" · "}
          ...
          {hiddenTotal > 0 && (
            <>
              ...
              <span className="text-warnInk" data-testid="stepgroup-hidden">
                {hiddenTotal} ocultos pelos seus filtros
              </span>
            </>
          )}
        </span>
          ...
          case "dirreq":
            ...
            return (
              <div className="approval-inline" key={bi}>
                ...
                <span>{item.resolved === "granted" ? "Acesso à pasta concedido" : "Acesso à pasta recusado"}</span>
          ...
          case "planreq":
            ...
            return (
              <div className="bubble-assistant" key={bi}>
                <div className="who">plano proposto</div>
                <Markdown text={item.plan} />
                <div className="approval-inline">
                  <span className={"status " + (item.resolved === "approved" ? "ok" : "denied")}>
                    {item.resolved === "approved" ? "✓" : "✕"}
                  </span>
                  <span>{item.resolved === "approved" ? "Plano aprovado" : "Devolvido com comentários"}</span>
                </div>
              </div>
            );
          case "notice":

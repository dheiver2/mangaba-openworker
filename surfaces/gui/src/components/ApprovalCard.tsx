...
export function approvalActionLabels(name?: string): { allow: string; deny: string } {
  return name === "save_skill"
    ? { allow: "Adicionar às minhas skills", deny: "Agora não" }
    : { allow: "Permitir uma vez", deny: "Negar" };
}
  ...
  return (
    <>
      ...
      <div className="approval-with">
        Ao aprovar, ela entra nas suas skills neste {deviceLabel()} — disponível em todas as
        conversas a partir de então.
      </div>
    </>
  );
}
...
export function scopeNote(
  ...
  if (name === "save_skill") return { text: "salva em Configurações ▸ Skills", external: false };

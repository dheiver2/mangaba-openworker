...
const openWriteForm = async () => {
  fireEvent.click(await screen.findByRole("button", { name: /Adicionar skill/ }));
  fireEvent.click(screen.getByText("Escrever eu mesmo"));
};
...
describe("SkillsTab", () => {
  ...
  it("blocks Save until name and instructions are filled", async () => {
    ...
    render(<SkillsTab />);
    await openWriteForm();
    const save = screen.getByText("Salvar skill") as HTMLButtonElement;
    expect(save.disabled).toBe(true);
    fireEvent.change(screen.getByLabelText("Nome"), { target: { value: "greet" } });
  ...
  it("creates a skill (global, no scope field) and refreshes the list", async () => {
    ...
    fireEvent.click(screen.getByText("Salvar skill"));
    await waitFor(() => {
  ...
  it("edit prefills the form (name locked, body loaded) and PATCHes on save", async () => {
    ...
    const body = screen.getByLabelText("Instruções") as HTMLTextAreaElement;
  ...
  it("delete is two-step: arm, then DELETE on confirm", async () => {
    ...
    fireEvent.click(screen.getByLabelText("Excluir weekly-report"));
    expect(calls.some((c) => c.method === "DELETE")).toBe(false);
    const confirm = await screen.findByText("Confirmar exclusão");
  ...
  it("the enabled switch PATCHes {enabled} and teaches the off rule + physics footnote", async () => {
    ...
    render(<SkillsTab />);
    await screen.findByText("weekly-report");
    fireEvent.click(screen.getByLabelText("weekly-report ativada"));
    await waitFor(() => {
      const patch = calls.find((c) => c.method === "PATCH");
    ...
    expect(status.textContent).toContain("desativada em todo lugar");
    expect(status.textContent).toContain("começar do zero"); // the guaranteed remedy, in place
  });
  ...
  it("upload shows the parsed preview and installs nothing until confirmed", async () => {
    ...
    render(<SkillsTab />);
    const input = (await screen.findByLabelText("Enviar um arquivo de skill")) as HTMLInputElement;
    const file = new File([new Uint8Array([80, 75, 3, 4])], "greet.zip", { type: "application/zip" });
    fireEvent.change(input, { target: { files: [file] } });
    await screen.findByText("Revise antes de instalar");
    ...
    fireEvent.click(screen.getByText("Instalar skill"));
    await waitFor(() => {
  ...
  it("Add skill menu: three doors; Create with OpenWorker hands off to a conversation", async () => {
    ...
    expect(screen.getByText("Escrever eu mesmo")).toBeTruthy();
    expect(screen.getByText("Importar um arquivo")).toBeTruthy();
    expect(screen.getByText(/você revisa antes de instalar/)).toBeTruthy();
    expect(screen.getByText(/pergunta antes de adicioná-la às\s+suas skills/)).toBeTruthy();
    fireEvent.click(screen.getByText("Criar com o Mangaba"));
  ...
  it("shows the new-session confirmation line after creating a skill", async () => {
    ...
    await openWriteForm();
    fireEvent.change(screen.getByLabelText("Nome"), { target: { value: "greet" } });
    fireEvent.change(screen.getByLabelText("Instruções"), { target: { value: "x" } });
    fireEvent.click(screen.getByText("Salvar skill"));
    const status = await screen.findByRole("status");
    expect(status.textContent).toContain("greet"); // name-first — WHICH skill
    expect(status.textContent).toContain("já pode usá-la em todas as conversas");
  });

  it("the list is the page: no standing add-surfaces, no drafting remnants", async () => {
    stubFetch([{ match: "/v1/skills", method: "GET", json: { skills: [] } }]);
    render(<SkillsTab onCreateSkill={vi.fn()} />);
    await screen.findByRole("button", { name: /Adicionar skill/ });
    ...
    await openWriteForm();
    expect(screen.queryByText("Escrever eu mesmo")).toBeNull();
    expect(screen.getByText("Salvar skill")).toBeTruthy();
  });
  ...
  it("surfaces server-side validation errors", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Instruções"), { target: { value: "y" } });
    fireEvent.click(screen.getByText("Salvar skill"));
...
describe("SkillsTab — rich-skill disclosure (§6)", () => {
  it("shows a file count only when a skill bundles resources", async () => {
    ...
    render(<SkillsTab />);
    const note = await screen.findByTitle("Mostrar pasta");
    expect(note.textContent).toContain("3 arquivos");
    // The one-file skill carries no count at all — only rich skills are marked.
    expect(screen.getAllByTitle("Mostrar pasta")).toHaveLength(1);
  });
});

...
declare const __MANGABA_DEV_TOKEN__: string;

// Endpoint resolution order: runtime-injected globals (Tauri sets `window.__MANGABA_HTTP__`
...
const httpBase = (): string =>
  (globalThis as any).__MANGABA_HTTP__ ||
  (import.meta as any).env?.VITE_MANGABA_HTTP ||
  "http://127.0.0.1:8765";
const wsBase = (): string =>
  (globalThis as any).__MANGABA_WS__ ||
  (import.meta as any).env?.VITE_MANGABA_WS ||
  "ws://127.0.0.1:8765";
const apiToken = (): string =>
  (globalThis as any).__MANGABA_API_TOKEN__ ||
...
const openWebSocket = (url: string): WebSocket => {
  const token = apiToken();
  // O navegador não deixa passar headers no handshake do WebSocket, então o token
  // viaja como subprotocolo (o servidor responde apenas "mangaba").
  const protocols = ["mangaba", token].filter(Boolean) as string[];
...
// --- Conexões manuais ---
//
// A "Mangaba Cloud" (login OAuth + one-click gerenciado via broker) foi removida
// deste fork — a infraestrutura não existe. Cada conector é conectado colando a
// credencial do próprio fornecedor (bot_token, api_token, access token). O
// connect MCP local (abaixo) segue disponível: roda inteiro neste computador,
// sem conta Mangaba.
...
export const INBOX_UNLOCK = "mangaba:inbox-unlock";
...
export const PERSONAS_CHANGED = "mangaba:personas-changed";
...
// Solo page for one gallery mangaba. `capabilities` is the desktop's own
...
export const AUTOMATIONS_CHANGED = "mangaba:automations-changed";
...
// bot is a member (Slack API constraint); is_member=false → "invite @Mangaba" hint.

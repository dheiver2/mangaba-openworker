...
it("authenticates REST and session WebSocket calls with the launch token", async () => {
  vi.stubGlobal("__MANGABA_API_TOKEN__", "launch-token");
  const request = vi.fn(async (_url: string, init?: RequestInit) => {
    expect(new Headers(init?.headers).get("X-Mangaba-Token")).toBe("launch-token");
  ...
  expect(socket.protocols).toEqual(["mangaba", "launch-token"]);
});

...
#   3. `tauri build --bundles app` → Mangaba.app (resources are copied in).
...
#   MANGABA_EXPERIMENTAL=1 ./build_dmg.sh
set -euo pipefail
...
rm -rf "$GUI/src-tauri/binaries/sidecar" "$GUI/src-tauri/binaries/mangaba-server-$TRIPLE"
...
cp -RL "$HERE/dist/mangaba-server" "$GUI/src-tauri/binaries/sidecar"
...
chmod +x "$GUI/src-tauri/binaries/sidecar/mangaba-server"
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "mangaba-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/mangaba-server"
fi

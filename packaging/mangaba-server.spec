...
"""PyInstaller spec for the bundled `mangaba-server` (desktop sidecar).
...
archive to a temp dir on EVERY launch — 6-7s of "Starting mangaba…" splash (measured; the
actual Python import is ~0.5s). The wrinkles handled here:
  - aisuite is a regular pip dependency (git-pinned in pyproject.toml); collect mangaba +
...
# is stripped, not just disabled. Self-builders opt in with MANGABA_EXPERIMENTAL=1; the
# loader in mangaba/connectors/descriptors.py treats the missing package as a no-op.
INCLUDE_EXPERIMENTAL = os.environ.get("MANGABA_EXPERIMENTAL") == "1"

hiddenimports = []
datas = []
binaries = []

for pkg in ("mangaba", "aisuite", "mcp", "ddgs", "croniter", "docstring_parser"):
    hiddenimports += collect_submodules(pkg)

if not INCLUDE_EXPERIMENTAL:
    hiddenimports = [
        m for m in hiddenimports if not m.startswith("mangaba.connectors.experimental")
    ]
...
a = Analysis(
    ...
    + ([] if INCLUDE_EXPERIMENTAL else ["mangaba.connectors.experimental"]),
    noarchive=False,
)
pyz = PYZ(a.pure)
exe = EXE(
...
# Onedir: dist/mangaba-server/{mangaba-server[.exe], _internal/}. The build scripts stage

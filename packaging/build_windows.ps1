...
<#
.SYNOPSIS
  Build the Mangaba Windows desktop app + NSIS (.exe) and MSI installers.
...
.DESCRIPTION
  The Windows counterpart to build_dmg.sh:
    ...
    3. `tauri build --bundles nsis,msi` -> Mangaba NSIS setup .exe + .msi (resources copied in).
  ...
  the spec strips mangaba.connectors.experimental. Self-builders can opt in with:
    $env:MANGABA_EXPERIMENTAL = "1"; .\build_windows.ps1
#>
[CmdletBinding()]
param(
...
# A running mangaba-server.exe (e.g. a prior sidecar/smoke test) locks the output exe and
# makes PyInstaller's overwrite fail with Access-is-denied. Stop any before bundling.
$running = Get-Process -Name "mangaba-server" -ErrorAction SilentlyContinue
if ($running) {
    Write-Host "==> stopping $($running.Count) running mangaba-server process(es) holding the output exe"
    $running | Stop-Process -Force
    Start-Sleep -Seconds 1
}

Write-Host "==> [1/3] PyInstaller: bundling mangaba-server ($Triple)" -ForegroundColor Cyan
& $PyInst --noconfirm --clean `
    --distpath (Join-Path $Here "dist") --workpath (Join-Path $Here "build") `
    (Join-Path $Here "mangaba-server.spec")
...
$Src = Join-Path $Here "dist\mangaba-server"
$Dst = Join-Path $BinDir "sidecar"
if (Test-Path $Dst) { Remove-Item -Recurse -Force $Dst }
# Clear any stale onefile binary from pre-onedir builds.
Remove-Item -Force (Join-Path $BinDir "mangaba-server-$Triple.exe") -ErrorAction SilentlyContinue
